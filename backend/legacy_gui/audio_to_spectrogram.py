...
from backend.maistro.cleanup import delete_generated_files
...
RECORDING_PATH = config.PROJECT_ROOT / "recording.mid"
COLLAB_MIDI_PATH = config.PROJECT_ROOT / "collab_output.mid"
...
def start_recording(duration=15):
    ...
    midi_file.save(str(RECORDING_PATH))
    print(f"MIDI recording saved as {RECORDING_PATH}")

    # extend_midi(RECORDING_PATH, COLLAB_MIDI_PATH, additional_intervals=15) # algorithmic extension
    generate_collab(recording_path=RECORDING_PATH, output_path=COLLAB_MIDI_PATH) # ai extension

    # Once collab output is created, signal that fireworks should stop updating.
    recording_finished = True
    request = 'c'
    process_audio_and_start(COLLAB_MIDI_PATH)
...
# Load PNG overlays
overlay_image1 = pygame.image.load(str(config.GUI_ASSETS_DIR / "logo.png")).convert_alpha()
overlay_image1 = pygame.transform.scale(overlay_image1, (150, 150))
overlay_image2 = pygame.image.load(str(config.GUI_ASSETS_DIR / "slur.png")).convert_alpha()
...
def process_audio_and_start(midi_path: Path):
    print("request: " + request)
    mode = "generation" if request == 'g' else "collaboration"
    wav_path = render_midi_to_wav(midi_path, mode=mode)
    file_ext = wav_path.suffix[1:]
    global audio, sample_rate, samples, WATERFALL_FRAMES, FREQ_VECTOR, freq_mask, num_freq_bins, waterfall_image_data
    audio = AudioSegment.from_file(str(wav_path), format=file_ext).set_channels(1)
    ...
    audio_thread = threading.Thread(target=play_audio, args=(wav_path,), daemon=True)
    audio_thread.start()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if button1_1.collidepoint(event.pos):
                ...
                def generate_midi():
                    generated_path = generate('generated_output.mid') # ai generation
                    # random_scale = random.randint(59, 70)  # random root note within a musical range
                    # generated_path = config.OUTPUT_DIR / 'generated_output.mid'
                    # generate_music(random_scale, str(generated_path)) # algorithmic generation
                    global request,active_button2
                    request = 'g'
                    process_audio_and_start(generated_path)
                    active_button2 = None
